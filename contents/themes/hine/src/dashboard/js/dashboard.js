...
lobby.dash = {  
  ...
  addTiles : function(){
     ...
     */
    $(".tiles[data-page="+ curPage +"]").append('<div class="tile"><div class="extra" id="lobby-store" data-mode="none" data-initdelay="50"><a href="'+ lobby.url +'/admin/lobby-store.php"><div class="inner"><div class="image"><img src="<?THEME_URL?>/src/dashboard/image/get-it.svg" height="100%" width="100%"/></div><div class="title">Get More Apps From<br/>Lobby Store</div></div></a></div></div>');
    ...
    setTimeout(function(){
      ...
      $.contextMenu({
        ...
        items: {
          ...
          admin: {name: "Go To Admin", icon: "open", callback: function(key, opt){ lobby.redirect("/admin/apps.php?app="+ $(this).attr("id")); }},
          disable: {name: "Disable App", icon: "close", callback: function(key, opt){ lobby.redirect("/admin/apps.php?action=disable&app="+ $(this).attr("id") +"&csrfToken="+ lobby.csrfToken); }},
          remove: {name: "Remove App", icon: "trash", callback: function(key, opt){ lobby.redirect("/admin/apps.php?action=remove&app="+ $(this).attr("id") +"&csrfToken="+ lobby.csrfToken); }}
        }
      });
    }, 10);
